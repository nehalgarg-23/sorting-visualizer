let array = [];
    ...
    function renderStep0(step){
      ...
      // stepDiv.textContent = `Step ${index }: `;
      step.forEach(value => {
        const block = document.createElement("div");
    ...
    function renderStep(step,stepNum,i,j){
      ...
      container.appendChild(stepDiv);
      document.querySelector(`.array-step${stepNum} .block${i}`).style.backgroundColor = "orange";
      document.querySelector(`.array-step${stepNum} .block${j}`).style.backgroundColor = "orange";
    }
